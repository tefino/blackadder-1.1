...
void Domain::writeClickFiles(bool montoolstub) {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        click_conf<<"classifier[1]->[0]cacheunit;" ;
        click_conf<<"classifier[2]->[1]cacheunit;" ;
        click_conf<<"classifier[3]->[2]cacheunit;" ;
        click_conf.close();
    }
}
...
void Domain::startTM() {
    ...
    pclose(ssh_command);
    /*now start the TM*/
    command = "ssh " + user + "@" + TM_node->testbed_ip + " \"/home/" + user + "/blackadder-1.0/TopologyManager/tm " + write_conf + "topology.graphml > /dev/null 2>&1 &\"";
    cout << command << endl;
    ssh_command = popen(command.c_str(), "r");
    if (ssh_command == NULL) {
